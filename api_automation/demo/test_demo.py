...
def login(s,userKey ,password):
    ...
    m=r1.json()["data"]["tokenInfo"]["token"]
    print(m)
    return m
if __name__=="__main__":
    # b=login(userKey=r.json()["data"]["key"],password=r.json()["data"]["key"])
    # print(b)
    s=requests.session()
    login_q()
   # print(b)
    b=login_q()
    print(b)
    k=login(s,userKey=login_q(),password=r1.json()["data"]["key"])
    print(k)
